...
def read_m6A_file(file_path):
	...
	# 使用染色体、位置和链作为唯一标识
	sites = set(zip(df['Chromosome'], df['m6A_site'], df['Strand']))
	return sites
...
def calculate_overlap(input_m6Apos_file, input_refpos_file):
	...
	venn2(subsets=(len(only_m6A), len(only_ref), len(overlap)),
		  set_labels=('pvalue m6A sites', 'Reference sites'))
	plt.title('Overlap between pvalue m6A sites and Reference sites')
	
	# 保存结果
	output_overlap = input_m6Apos_file.replace('.tsv', '_overlap.bed')
	output_m6A_specific = input_m6Apos_file.replace('.tsv', '_specific.bed')
	...
	for df in [overlap_df, m6A_specific_df]:
		df['start'] = df['pos'] - 1  # BED格式是0-based
		df['name'] = 'm6A_site'
		df['score'] = '.'
		df = df[['chrom', 'start', 'pos', 'name', 'score', 'strand']]
	
	# 保存文件
	overlap_df.to_csv(output_overlap, sep='\t', index=False, header=False)
	m6A_specific_df.to_csv(output_m6A_specific, sep='\t', index=False, header=False)
